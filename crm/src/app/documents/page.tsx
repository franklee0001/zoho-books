...
const docTypeLabelKey: Record<string, string> = {
  packing_list: "documents.packingList",
  commercial_invoice: "documents.commercialInvoice",
...
export default async function DocumentsPage({
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">{t(locale, "documents.title")}</h1>
        <p className="text-sm text-gray-500 mt-1">
          {t(locale, "documents.subtitle", { count: total.toLocaleString() })}
        </p>
      </div>

      {error && (
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm">
          <p className="font-semibold text-amber-800">{t(locale, "common.dbError")}</p>
      ...
      <form method="GET" className="mb-4">
        <div className="flex gap-3">
          <input
            ...
            name="q"
            defaultValue={query}
            placeholder={t(locale, "documents.searchPlaceholder")}
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {DOC_TYPE_OPTIONS.map((dt) => {
          const isActive = docTypeFilter === dt;
          const label = dt === "all" ? t(locale, "common.all") : (docTypeLabelKey[dt] ? t(locale, docTypeLabelKey[dt]) : dt);
          return (
            <a
              key={dt}
              href={buildHref(query, dt, 1)}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="px-6 py-3 text-left font-semibold text-gray-600">{t(locale, "documents.fileName")}</th>
                <th className="px-6 py-3 text-left font-semibold text-gray-600">{t(locale, "documents.type")}</th>
                <th className="px-6 py-3 text-left font-semibold text-gray-600">{t(locale, "documents.invoice")}</th>
                <th className="px-6 py-3 text-left font-semibold text-gray-600">{t(locale, "documents.customer")}</th>
                <th className="px-6 py-3 text-left font-semibold text-gray-600">{t(locale, "documents.generated")}</th>
                <th className="px-6 py-3 text-right font-semibold text-gray-600">{t(locale, "documents.size")}</th>
                <th className="px-6 py-3 text-center font-semibold text-gray-600">{t(locale, "documents.download")}</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={doc.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {docTypeLabelKey[doc.doc_type] ? t(locale, docTypeLabelKey[doc.doc_type]) : doc.doc_type}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <Link
              ...
              {documents.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-gray-400">
                    {error
                      ? t(locale, "documents.connectDb")
                      : t(locale, "documents.noDocuments")}
                  </td>
                </tr>
              )}
            </tbody>
